...
ment_t ment_top[] = {
	MDEF_HANDLER("Launch payload.bin", launch_payload_bin, colors[0]),
	MDEF_HANDLER("Launch bootloader/update.bin", launch_update_bin, colors[1]),
	MDEF_HANDLER("Launch atmosphere/reboot_payload.bin", launch_r2p_bin, colors[2]),
	MDEF_HANDLER("Other payloads...", launch_tools, colors[3]),
	MDEF_CAPTION("---------------", colors[4]),
	MDEF_HANDLER_EX("Reboot (OFW)", &STATE_REBOOT_BYPASS_FUSES, power_set_state_ex, colors[5]),
	MDEF_HANDLER_EX("Reboot (RCM)", &STATE_REBOOT_RCM, power_set_state_ex, colors[0]),
	MDEF_HANDLER_EX("Power off", &STATE_POWER_OFF, power_set_state_ex, colors[1]),
	MDEF_END()
};
...
void ipl_main()
{
	...
	// Grey out payload.bin option if no payload.bin found.
	if (f_stat("payload.bin", NULL))
	{
		grey_out_menu_item(&ment_top[0]);
	}

	// Grey out update.bin option if no update.bin found.
	if (f_stat("bootloader/update.bin", NULL))
	{
		grey_out_menu_item(&ment_top[1]);
	}

	// Grey out r2p option if no reboot_payload.bin found.
	if (f_stat("atmosphere/reboot_payload.bin", NULL))
	{
